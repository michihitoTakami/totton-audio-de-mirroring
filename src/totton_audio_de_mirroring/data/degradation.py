...
def upsample_bessel_reference(
    ...
    """Upsample signal with Bessel IIR path used as project reference SRC.

    Args:
        signal: Input signal (1D or 2D).
        source_sr: Source sample rate in Hz.
        target_sr: Target sample rate in Hz.
        cutoff_hz: Low-pass cutoff in Hz at target rate.
        order: Bessel IIR order.
    ...
    Physical Basis:
        Project scope assumes Stage 1 input is generated by a Bessel-like
        SRC path. This function provides a single canonical reference route.
    """
    _validate_signal(signal)
    _validate_sample_rate(source_sr)
    _validate_sample_rate(target_sr)
    ratio = _validate_upsample_ratio(source_sr, target_sr)
    return _upsample_iir(signal, ratio, target_sr, cutoff_hz, order, "bessel")
